...
// ─── GEMINI API HELPER ───
// ─── SECURE AI HELPER — calls our Next.js backend (never exposes key) ───
...
function AssessmentPage({ t, lang }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const raw = await callGemini(`You are an English assessment AI for a call center platform. The user completed the "${section.id}" section and scored ${score}%. CEFR level: ${getLevel(score).lvl}. Respond with ONLY valid JSON (no markdown):
{"feedback":"<2-3 sentences: honest assessment of their level, 1 strength, 1 improvement tip. Max 80 words.>"}`);
      const clean = raw.replace(/```json|```/g, "").trim();
      const parsed = JSON.parse(clean);
      setAiFeedback(parsed.feedback || "Good effort! Keep practicing to improve your score.");
      setFullResult({ score, grammarScore, vocabScore, readingScore, listeningScore, aiFeedback: parsed.feedback, strengths, weaknesses, candidateName: "" });
    } catch {
      const fb = "Good effort on this section! Your responses show solid understanding. Focus on the weak areas identified to reach the next CEFR level.";
      setAiFeedback(fb);
...
function HRPage({ t, onBack, lang }) {
  ...
  const analyze = async () => {
    ...
    try {
      const raw = await callAI(`You are a strict HR interviewer at a call center company. Evaluate this interview answer honestly.
...
{"score":NUMBER,"feedback":"One specific weakness and one concrete improvement tip. Max 50 words."}`, 300);
      ...
      const fallback = wordCount < 10 ? 28 : wordCount < 25 ? 48 : 58;
      setAnswerScore(fallback);
      setFeedback("AI is temporarily unavailable. Your answer was scored based on length and structure. Try to include specific examples and professional language.");
      setScores(p => [...p, fallback]);
    }
    setLoading(false);
  };
            ...
            <div style={{
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                ...
                <div style={{
                  ...
                  color: answerScore >= 75 ? "var(--emerald)" : answerScore >= 50 ? "var(--amber)" : "var(--rose)"
                }}>{answerScore}%</div>
              </div>
...
function SpeakingSection({ t, prompts, timer: globalTimer, onBack, lang }) {
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      const raw = await callAI(`You are a professional HR interviewer evaluating a call center job candidate. Be STRICT and HONEST.
...
{"grammarScore":N,"fluencyScore":N,"vocabScore":N,"overallScore":N,"feedback":"1 specific weakness + 1 actionable tip. Max 50 words.","corrected":"Model answer a strong candidate would give. 2-3 sentences.","pronunciationNote":"1 note on likely pronunciation challenge based on their word choices."}`, 500);
      ...
      const fb = { question: q.q, answer: trimmed, grammarScore: 5, fluencyScore: 5, vocabScore: 5, overallScore: 55, feedback: "AI evaluation temporarily unavailable. Focus on specific examples and professional vocabulary.", corrected: "", pronunciationNote: "" };
